...
import vn.minerva.travinh.func.medical_detail.domain.MedicalDetailImageListMapper
import vn.minerva.travinh.func.medical_detail.domain.MedicalDetailRootUseCase

class MedicalDetailPresenter(var screenNavigator: AndroidScreenNavigator) :MedicalDetailContract.Presenter(){
    override fun gotoMedicalListActivity() {
    ...
    override fun getMedicalInfo(id: Int) {
        ...
        useCaseTask = medicalDetailRootUseCase.executeAsync(object :
            ...
            override fun success(data: MedicalDetailRootUseCase.Output) {
                if (data.medicalDetailResponse.success) {
                    view?.showImageList(MedicalDetailImageListMapper().map(data.medicalDetailResponse))
                    view?.showDetailToolbar(data.medicalDetailResponse)
                } else {

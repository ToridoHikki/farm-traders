...
class DecimalUtils {
    companion object {
        ...
        enum class RoundMode {
            ...
            companion object {
                ...
                fun getRoundMode(roundMode: RoundMode): Int {
                    val mode = when (roundMode) {
                        UP -> BigDecimal.ROUND_CEILING
                        DOWN -> BigDecimal.ROUND_FLOOR
                        NATURAL -> BigDecimal.ROUND_HALF_UP
                    }
                    return mode
                }
            }
        ...
        fun decimalFormat(value: Double, numberAfterDot: Int, locale: Locale
            ...
            if (groupDecimalMode != Companion.GroupDecimalMode.LOCALE) {
                if (groupDecimalMode == Companion.GroupDecimalMode.DOT) {
                    symbols.groupingSeparator = '.'
                    symbols.decimalSeparator = ','
                } else if (groupDecimalMode == Companion.GroupDecimalMode.COMMA) {
                    symbols.groupingSeparator = ','
                    symbols.decimalSeparator = '.'
                }
            }
            ...
            if (numberAfterDot > 0) {
                var patternAfterDot = "#"
                if (afterDotMode == Companion.AfterDotMode.ALWAYS_ZERO) {
                    patternAfterDot = "0"
                }
                pattern.append(".")
        ...
        fun roundNumber(value: Double, numberDigit: Int, roundMode: RoundMode): Double {
            return BigDecimal.valueOf(value).setScale(numberDigit, Companion.RoundMode.getRoundMode(roundMode)).toDouble()
        }
    }
}